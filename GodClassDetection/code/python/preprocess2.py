...
def get_xy_train1(path, item, mn_maxlen, tokenizer, embedding_matrix):
...
def get_xy_test1(path, item, mn_maxlen, tokenizer, embedding_matrix):
...
# def get_xy_test(path, item, maxlen, tokenizer, embedding_matrix):
#     labels = []
#     for project in sorted(os.listdir(path)):
#         if (project == item):
#             lb_path = path + "/" + project + "/lb_train.txt"  # D:\卜依凡\学习\论文-检测God Class\数据集\trainset_0707\manual-freeplane
