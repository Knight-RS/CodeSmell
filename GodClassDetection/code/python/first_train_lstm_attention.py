...
def attention_block(inputs, feature_cnt, dim):
    a = Flatten()(inputs)
...
lstm_share = LSTM(output_dim=50, activation='sigmoid', init='uniform')  # LSTM的输出层，激活函数，初始化

embedding_a = masking_layer(method_a)  # 嵌入层
lstm_a = lstm_share(embedding_a)

embd = Embedding(100000, EMBEDDING_DIM)(metric_a)
embd = BatchNormalization()(embd)
hidden = attention_block(embd, 12, EMBEDDING_DIM)
# hidden = Lambda(lambda x: K.mean(x, axis=1))(hidden)
hidden = Flatten()(hidden)

# # ATTENTION PART STARTS HERE 注意力层
# attention_probs = Dense(MAX_SEQUENCE_LENGTH, activation='softmax', name='attention_vec')(lstm_a)
# attention_mul = Multiply()([lstm_a, attention_probs])
# # ATTENTION PART FINISHES HERE


dense_share2 = Dense(12, activation='tanh', init='uniform')  # 全连接层
mtrdense_a = dense_share2(hidden)
...
with open(r'/Users/knight/Desktop/GodClassDetection-master-mao-new/trained_model/pre_training_lstm_att_model.json',
          'w') as file:
    file.write(model_json)

# 保存预训练的模型
model_final.save('/Users/knight/Desktop/GodClassDetection-master-mao-new/trained_model/pre_training_lstm_att_model.h5')
