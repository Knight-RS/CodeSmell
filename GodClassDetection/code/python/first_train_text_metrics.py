...
tokenizer = preprocess0809.get_tokenizer(TRAIN_SET_DIR)
all_word_index = tokenizer.word_index
embedding_matrix = preprocess0809.get_embedding_matrix(all_word_index, W2V_MODEL_DIR, dim=EMBEDDING_DIM)

epochs = 10
acc_list = []
loss_list = []

x_train, y_train = preprocess0809.get_xy_train(TRAIN_SET_DIR + '/data', tokenizer=tokenizer,
...
metric = preprocess0809.get_metrics_up()
# print(len(metric))
# print(metric.shape)
c1 = []
c2 = []
...
x_train_t.append(x_train[4])
x_train_t.append(x_train[5])
x_train_t.append(x_train[6])
...
def build_interation(att, filters, units):
    ...
    for _ in range(3):
        att_output = interaction1.InteractingLayer(8, 2, True)(att)
    # print(att_output.shape)
    # print(att_output)

    att_output = Conv1D(filters, 1, padding="same", activation='tanh')(att_output)
    # print(att_output)
    att_output = Conv1D(filters, 1, activation='tanh')(att_output)
    # print(att_output)
    att_output = Conv1D(filters, 1, activation='tanh')(att_output)
    # print(att_output)
    ...
    interaction = Dense(units, activation='tanh')(att_output)
    return interaction


# interaction_a = build_interation(att_a, 128, 32)
# interaction_b = build_interation(att_b, 128, 4)
interaction_c1 = build_interation(att_c1, 63, 4)
interaction_c2 = build_interation(att_c2, 64, 4)
interaction_c3 = build_interation(att_c3, 64, 4)
...
# # print("interaction_b_shape:", interaction_b.shape)
# print("interaction_c1_shape:", interaction_c1.shape)
# print("interaction_c2_shape:", interaction_c2.shape)
# print("interaction_c3_shape:", interaction_c3.shape)

# 合并三个输入

m_j_merged_a = concatenate(
    [lstm_a, mtrdense_a, interaction_c1, interaction_c2, interaction_c3], axis=-1)
...
model_final = Model(inputs=[method_a, metric_a, att_c1, att_c2, att_c3], output=total_output)
...
hist = model_final.fit(x_train_t, y_train, nb_epoch=epochs, batch_size=5, verbose=1)
score = model_final.evaluate(x_train_t, y_train, verbose=0)
model_final.summary()
...
with open(r'E:/GodClassDetection/trained_model/pre_training_model_metrics_int_10.json', 'w') as file:
    file.write(model_json)

# 保存预训练的模型
model_final.save('E:/GodClassDetection/trained_model/pre_training_model_metrics_int_10.h5')
