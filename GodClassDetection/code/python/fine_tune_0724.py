...
# model_path = '/Users/knight/Desktop/GodClassDetection/trained_model/pre_training_model_int_100.h5'
...
for project in sorted(os.listdir(data_path)):
    ...
    print("path:", path)
    x_train, y_train = preprocess2.get_xy_train1(TRAIN_SET_DIR + '/finetune', project, mn_maxlen=MAX_SEQUENCE_LENGTH,
                                                 tokenizer=tokenizer,
    ...
    # 训练集度量值
    hist = model.fit(x_train, y_train, nb_epoch=epochs, batch_size=5, verbose=2)  # 1为输出进度条记录，2为每个epoch输出一行记录
