...
def get_data(path, maxlen, tokenizer, embedding_matrix):
    mn_datas = []
    train_projects = []
    mn_path = path + "/mn_train.txt"  # D:\卜依凡\学习\论文-检测God Class\数据集\trainset_0707\manual-freeplane
    # print("mn_path:", mn_path)
    with open(mn_path) as f:
        for line in f:
            data_mn_list = line.strip('\n').strip('\r').strip().split('.')
...
def get_labels(path):  # 01标签
    labels = []
    lb_path = path + "/lb_train.txt"  # D:\卜依凡\学习\论文-检测God Class\数据集\trainset_0707\manual-freeplane
    # print("lb_path:", lb_path)
    with open(lb_path) as f:
        for line in f:
            labels.append(int(line))
...
def get_metrics(path):  # 12个度量
    metrics_datas = []
    mt_path = path + "/mt_train.txt"  # D:\卜依凡\学习\论文-检测God Class\数据集\trainset_0707\manual-freeplane
    # print("mt_path:", mt_path)
    with open(mt_path) as f:
        for line in f:
            metrics = line.split()
...
def get_xy_test(path, item, test_project, maxlen, tokenizer, embedding_matrix):
    labels = []
    for project in sorted(os.listdir(path)):
        if (project == item):
            lb_path = path + "/" + project + "/lb_train.txt"  # D:\卜依凡\学习\论文-检测God Class\数据集\trainset_0707\manual-freeplane
    ...
    x_val.append(np.asarray(metrics_datas))
    y_val = labels
    return x_val, y_val
